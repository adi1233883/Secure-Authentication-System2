// AI Service — wraps all Anthropic Claude API calls. If no API key is
...
if (env.isAiConfigured) {
  ...
  logger.warn('[ai] ANTHROPIC_API_KEY not set — AI features will return fallback responses. Add a key to .env to enable them.');
}

const FALLBACK_MESSAGE =
  "The AI assistant isn't configured yet. Add an ANTHROPIC_API_KEY to the backend .env file to enable this feature.";
...
async function askSecurityAssistant(userMessage, conversationHistory = []) {
  ...
  try {
    const response = await client.messages.create({
      model: MODEL,
      max_tokens: 500,
      system: `You are a friendly, knowledgeable cybersecurity assistant embedded in a "Secure Authentication System" demo project.
...
async function getRecommendations(context) {
  ...
  try {
    const response = await client.messages.create({
      model: MODEL,
      max_tokens: 400,
      system: `Generate exactly 3 short, specific, actionable security recommendations for a user of an authentication
...
async function summarizeFindings(findings) {
  if (!client) {
    return {
      summary: 'AI summarization is not configured. Showing raw findings only — add an ANTHROPIC_API_KEY to enable plain-English summaries.',
      aiConfigured: false,
    };
  }
  ...
  try {
    const response = await client.messages.create({
      model: MODEL,
      max_tokens: 600,
      system: `You are a security analyst assistant. You will receive structured findings already detected by
