...
on the frontend, with **Anthropic Claude** powering the AI Security Assistant,
...
- (Optional) An **Anthropic API key** from https://console.anthropic.com — the
...
5. **Run the AI Log Analyzer**: go to Security Analytics → "Re-analyze
   logs". With a real `ANTHROPIC_API_KEY` set, you'll get a plain-English
...
| AI chatbot says it's "not configured" | Add `ANTHROPIC_API_KEY` to backend `.env` and restart the server |
